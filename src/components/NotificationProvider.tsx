...
function LunaNotification({ item, onOpen }: { item: AppNotification; onOpen: () => void }) {
  return <button onClick={onOpen} className="flex w-full items-start gap-3 text-left"><LunaAvatar size="sm" mood="warm" /><span className="min-w-0 flex-1"><span className="block text-sm font-semibold">{item.title}</span><span className="mt-0.5 block text-xs leading-relaxed text-muted-foreground">{item.body}</span></span></button>;
}
