...
export const Route = createFileRoute("/exam-focus")({
  head: () => ({ meta: [{ title: "Exam Focus — MindMate" }, { name: "description", content: "Quiet, focused study sessions with Luna by your side." }] }),
  component: ExamFocus,
});

function ExamFocus() {
  const [preset, setPreset] = useState(examPresets[0]);
    ...
    <Shell>
        ...
        <div className="glass-strong relative flex flex-col items-center overflow-hidden rounded-3xl p-8 glow-purple">
          ...
          <div className="relative">
            <svg className="h-56 w-56 -rotate-90" viewBox="0 0 200 200">
              <circle cx="100" cy="100" r="88" stroke="currentColor" strokeWidth="6" fill="none" className="text-white/10" />
              <circle cx="100" cy="100" r="88" stroke="url(#g)" strokeWidth="6" fill="none" strokeLinecap="round" strokeDasharray={`${pct * 5.52} 1000`} />
              <defs>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <p className="mt-1 text-[10px] uppercase tracking-wider text-muted-foreground">{preset.label}</p>
            </div>
          </div>

          <div className="relative mt-6 flex gap-3">
            <button onClick={() => { setRunning(false); setSecondsLeft(preset.focus * 60); }} className="glass flex h-12 w-12 items-center justify-center rounded-full">
              <RotateCcw className="h-5 w-5" />
            </button>
            <button onClick={() => setRunning((r) => !r)} className="flex h-14 w-14 items-center justify-center rounded-full gradient-primary glow-cyan">
              {running ? <Pause className="h-6 w-6 text-white" fill="currentColor" /> : <Play className="ml-1 h-6 w-6 text-white" fill="currentColor" />}
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 pt-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {examPresets.map((p) => (
            <button key={p.id} onClick={() => setPreset(p)} className={`glass rounded-2xl p-3 text-center ${preset.id === p.id ? "ring-2 ring-purple" : ""}`}>
              <p className="text-sm font-bold">{p.focus}/{p.brk}</p>
