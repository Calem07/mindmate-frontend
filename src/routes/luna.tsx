...
export const Route = createFileRoute("/luna")({
  head: () => ({
    meta: [
      { title: "Luna — Your companion" },
      {
        name: "description",
        content: "Chat with Luna, your AI companion who grows alongside you.",
      },
    ],
  }),
...
function buildContextSuggestions(context: LunaContext | null): Suggestion[] {
  const out: Suggestion[] = [];
  const lastCheckIn = context?.latestCheckIn;
  const mood = lastCheckIn?.mood;
  const tags = lastCheckIn?.tags ?? [];
  const skipped = (context?.todayHabits ?? []).filter((h) => h.status === "not_started");
  ...
  if (mood === "low" || mood === "rough") {
    out.push({
      ...
      prompt: "Yesterday felt really heavy. Can we ease into today together?",
    });
  } else if (mood === "okay") {
    out.push({
      label: "Help me lift today",
      reason: "Yesterday was just okay",
      prompt: "Yesterday was just okay. Help me find one small bright thing for today?",
    });
  } else if (mood) {
    out.push({
      label: "Protect this momentum",
      reason: `Yesterday felt ${mood}`,
      prompt: `Yesterday felt ${mood}. How do I protect this energy today?`,
    });
  }
  if (lastCheckIn?.sleepHours != null && lastCheckIn.sleepHours < 7) {
    out.push({
      label: "I slept badly",
      reason: `Only ${lastCheckIn.sleepHours}h of sleep`,
      prompt: `I only slept ${lastCheckIn.sleepHours} hours. What should today look like?`,
    });
  }
  if (tags.includes("exam-stress") || tags.includes("anxious")) {
    out.push({
      ...
      prompt: "My exam nerves are climbing. Can you help me ground?",
    });
  }
  if (skipped.length > 0) {
    out.push({
      label: `Nudge me on ${skipped[0].name.toLowerCase()}`,
      reason: "Not started today",
      prompt: `I haven't started "${skipped[0].name}" yet. Gently nudge me?`,
    });
  }
  if (hour < 11)
    out.push({
      label: "Set a soft intention",
      reason: "Morning",
      prompt: "Help me set one soft intention for today.",
    });
  else if (hour >= 21)
    out.push({
      label: "Help me wind down",
      reason: "Late evening",
      prompt: "Help me wind down for the night.",
    });
...
function LunaScreen() {
  ...
  const [pending, setPending] = useState<{ kind: "image" | "audio" | "file"; name: string } | null>(
    null,
  );
  const [searchOpen, setSearchOpen] = useState(false);
  const [search, setSearch] = useState("");
  const [lunaContext, setLunaContext] = useState<LunaContext | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [rows, context] = await Promise.all([lunaApi.history(), lunaApi.context()]);
        if (cancelled) return;
        ...
        if (!rows.length) {
          ...
          setMessages([
            {
              id: uid(),
              from: "luna",
              text: `Hey ${firstName}. I've been thinking about you. How are you, *really*?`,
          ...
          setMessages(
            rows.map((r) => ({
              id: r.id,
              from: r.role.toLowerCase() === "user" ? "me" : "luna",
              text: r.content,
              time: fmtTime(new Date(r.createdAt)),
  ...
  const contextSuggestions = useMemo(() => buildContextSuggestions(lunaContext), [lunaContext]);
  ...
  const nudges = useMemo(() => {
    ...
    if (daysSinceCheckIn === null || !isSameDay) {
      items.push({
        id: "checkin-today",
        icon: ClipboardCheck,
        title:
          daysSinceCheckIn === null || daysSinceCheckIn === 0
            ...
            : daysSinceCheckIn === 1
              ...
              : `It's been ${daysSinceCheckIn} days`,
    ...
    if (streakDays >= 3) {
      ...
      if (hour >= 18 && !isSameDay) {
        items.push({
          ...
          prompt: `My ${streakDays}-day streak is at risk tonight. Help me protect it in the smallest way possible.`,
        ...
        items.push({
          ...
          prompt: `Celebrate my ${streakDays}-day streak with me in one gentle sentence.`,
          cta: "Say something soft",
        });
      }
    }
    const skipped = (lunaContext?.todayHabits ?? []).filter((h) => h.status === "not_started");
    if (skipped.length > 0 && new Date().getHours() >= 15) {
      items.push({
        id: `skipped-${skipped[0].habitId}`,
        icon: Droplet,
  ...
  }, [dismissedNudges, lunaContext]);

  const dismissNudge = (id: string) => setDismissedNudges((s) => new Set(s).add(id));
  ...
  const sendPrompt = async (text?: string) => {
    ...
    try {
      const res = await lunaApi.send(
        value || `Shared a ${pending?.kind ?? "note"}: ${pending?.name ?? ""}`,
      );
      const assistant = res.assistantMessage;

      setMessages((m) =>
        m
          .map((x) => (x.id === localId ? { ...x, status: "sent" as const } : x))
          .concat({
            id: assistant.id,
            from: "luna" as const,
            text: assistant.content,
            time: fmtTime(new Date(assistant.createdAt)),
            createdAt: new Date(assistant.createdAt),
            mood: assistant.mood as LunaMood,
  ...
  const toggleReaction = async (id: string) => {
    ...
    try {
      ...
      if (id.length > 20) await lunaApi.react(id, next);
    } catch {
      /* silent */
    }
  };
      ...
      <div className="px-5">
        <div className="glass-strong flex items-center gap-3 rounded-3xl p-3">
          ...
          <div className="flex flex-col items-end gap-0.5 text-[10px]">
            <span className="rounded-full bg-purple/20 px-2 py-0.5 font-semibold text-purple">
              LV {lunaContext?.profile?.level ?? "—"}
            </span>
            <span>
            ...
            <span>
              ...
              {messages.filter((m) => m.from === "luna").length}
            </span>
          </div>
        </div>
        ...
        {searchOpen && (
          <div className="glass mt-2 flex items-center gap-2 rounded-full px-3 py-2">
            ...
            />
            {search && (
              <button onClick={() => setSearch("")} className="text-xs text-muted-foreground">
                Clear
              </button>
            )}
          </div>
          ...
          <div className="space-y-2 animate-[rise_0.5s_ease-out]">
            {nudges.map((n) => {
              ...
              return (
                <div
                  key={n.id}
                  className="glass-strong relative overflow-hidden rounded-3xl p-3 pl-4 ring-1 ring-purple/20"
                >
        ...
        {groups.map(([day, msgs]) => (
          <div key={day} className="space-y-4">
            <div className="flex items-center justify-center">
              <span className="glass rounded-full px-3 py-1 text-[10px] uppercase tracking-wider text-muted-foreground">
                {day}
              </span>
            </div>
            {msgs.map((m) => (
              ...
              {showStarters
                ...
                : followupSuggestions.map((s) => (
                    <button
                      ...
                      className="glass rounded-full px-3 py-1.5 text-[11px] font-semibold text-purple"
                    >
                      <Sparkles className="mr-1 inline h-3 w-3" />
                      {s}
                    </button>
        ...
        {showStarters && (
          <div className="relative overflow-hidden rounded-3xl glass-strong">
            ...
            <div className="relative flex items-center justify-between p-4">
              ...
              <button
                onClick={() => sendPrompt("Guide me through one slow minute of breathing.")}
                className="flex h-11 w-11 items-center justify-center rounded-full gradient-primary glow-cyan"
              >
            ...
            <div className="glass-strong mb-2 flex items-center justify-around rounded-2xl p-3">
              {[
                ...
                <button
                  key={a.label}
                  onClick={() => {
                    setPending({ kind: a.kind, name: a.name });
                    setAttachOpen(false);
                  }}
          ...
          {pending && (
            <div className="glass-strong mb-2 flex items-center gap-2 rounded-full px-4 py-2 text-xs">
              {pending.kind === "image" ? (
                ...
                <Paperclip className="h-3.5 w-3.5 text-purple" />
              )}
...
function MessageBubble({
  ...
  return (
    <div
      className={`flex items-end gap-2 animate-[rise_0.4s_ease-out] ${isMe ? "flex-row-reverse" : ""}`}
    >
      {m.from === "luna" ? (
      ...
      <div className="max-w-[78%]">
        ...
        >
          ...
          {!isMe && (
            ...
            >
              <Heart
                className={`h-3.5 w-3.5 ${m.reacted ? "fill-purple text-purple" : "text-muted-foreground"}`}
              />
            </button>
          )}
        </div>
        <p
          className={`mt-1 flex items-center gap-1 text-[10px] text-muted-foreground ${isMe ? "justify-end" : ""}`}
        >
          {m.time}
          {m.status === "sending" && <span className="text-muted-foreground/70">· sending…</span>}
          {m.status === "sent" && isMe && (
            <span className="text-muted-foreground/70">· seen by Luna</span>
          )}
      ...
      <ReactMarkdown
        components={{
          a: ({ href, children }) => {
            if (href?.startsWith("action:")) {
              ...
              if (action) {
                ...
                return (
                  <Link
                    to={action.to}
                    className="mt-2 inline-flex items-center gap-2 rounded-full bg-purple/20 px-3 py-1.5 text-xs font-semibold text-purple no-underline hover:bg-purple/30"
                  >
            ...
            return (
              <a href={href} target="_blank" rel="noreferrer" className="text-purple underline">
                {children}
              </a>
            );
          },
          ...
          blockquote: ({ children }) => (
            <blockquote className="my-1 border-l-2 border-purple/40 pl-3 italic opacity-80">
              {children}
            </blockquote>
          ),
        }}
